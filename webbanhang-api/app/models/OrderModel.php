...
class OrderModel
{
    ...
    // Tạo đơn hàng mới (cập nhật thêm user_id)
    public function createOrder($fullname, $phone, $address, $note, $payment, $total, $userId = null)
    {
        // Kiểm tra cột user_id có tồn tại không, nếu chưa thì thêm
        $this->ensureUserIdColumn();

        $query = "INSERT INTO orders (fullname, phone, address, note, payment_method, total_amount, status, user_id, created_at)
                  VALUES (:fullname, :phone, :address, :note, :payment, :total, 'pending', :user_id, NOW())";
    ...
    // Lấy đơn hàng của 1 user (API mới)
    ...
    // Đảm bảo cột user_id tồn tại trong bảng orders
    private function ensureUserIdColumn(): void
    {
        try {
            $this->conn->query("SELECT user_id FROM orders LIMIT 1");
        } catch (PDOException $e) {
            $this->conn->exec("ALTER TABLE orders ADD COLUMN user_id INT DEFAULT NULL AFTER note");
        }
    }
}
?>
